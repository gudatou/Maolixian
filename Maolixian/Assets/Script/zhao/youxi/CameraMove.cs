...
public class CameraMove : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (cat_Transform.position.y > 14)
        {
            
            Vector3 nextPos = new Vector3(cat_Transform.position.x + 2, 14, cat_Transform.position.z - 10);
            m_Transform.position = Vector3.Lerp(m_Transform.position, nextPos, Time.deltaTime * 2);
        }
        if (cat_Transform.position.y < -110)
        {
            
            Vector3 nextPos = new Vector3(cat_Transform.position.x + 2, 110, cat_Transform.position.z - 10);
            m_Transform.position = Vector3.Lerp(m_Transform.position, nextPos, Time.deltaTime * 2);
        }
        if (cat_Transform.position.y > -110 && cat_Transform.position.y < 14)
        {
