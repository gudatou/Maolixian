...
public class camera2 : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (cat_Transform.position.y > 22)
        {

            Vector3 nextPos = new Vector3(cat_Transform.position.x + 2, 22, cat_Transform.position.z - 10);
            m_Transform.position = Vector3.Lerp(m_Transform.position, nextPos, Time.deltaTime * 2);
        }
        if (cat_Transform.position.y < -2)
        {

            Vector3 nextPos = new Vector3(cat_Transform.position.x + 2, -2, cat_Transform.position.z - 10);
            m_Transform.position = Vector3.Lerp(m_Transform.position, nextPos, Time.deltaTime * 2);
        }
        if (cat_Transform.position.y > -2 && cat_Transform.position.y < 22)
        {
