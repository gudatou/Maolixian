...
public class camera4 : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (cat_Transform.position.y > 50)
        {

            Vector3 nextPos = new Vector3(cat_Transform.position.x + 2, 50, cat_Transform.position.z - 10);
            m_Transform.position = Vector3.Lerp(m_Transform.position, nextPos, Time.deltaTime * 2);
        }
        if (cat_Transform.position.y < -4)
        {

            Vector3 nextPos = new Vector3(cat_Transform.position.x + 2, -4, cat_Transform.position.z - 10);
            m_Transform.position = Vector3.Lerp(m_Transform.position, nextPos, Time.deltaTime * 2);
        }
        if (cat_Transform.position.y > -3.7 && cat_Transform.position.y < 50)
        {
